...
/*
	...
	void ConfigureDecoder ()		/Put all the decoder #'s you need here.	Remember to change
	{ /
	...
	*/
	...
	void displaySubMenuA (int dispMenu)
	{
		...
		switch (dispMenu)
		{
			...
			case 2:
				...
				break;
			case 3:
				displaySubMenuFooter = String("4 Debug Mode");
				displayOutput(false,displaySubMenuHeader, displaySubMenuFooter);
				break;
			case 4:
				displaySubMenuFooter = String("5 Quit");
				displayOutput(false,displaySubMenuHeader, displaySubMenuFooter);
				break;
	...
	void manualMode()
	{
		...
		if (newMenu)
		{
			key = -1;
			newMenu = false;
			displayManualMove(currentTrack);
		}
...
//			lcd.write(byte(3));
	...
	void displayResultCalibration(int nRuns, boolean isEnd)
	{
		...
		if (isEnd)
		{
			...
			runs = 0;
			currentStepPosition = aveRefValue;
			overwriteTrack();
...
/*	//  ------------------------------------
		...
		void doStepperMove()
		{
			...
			boolean newTargetSet = false;
				...
				if (newTargetLocation)			 // If there is a new target location, set the target
				{
