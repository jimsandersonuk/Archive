...
class LCD_Turntable extends Arduino{//Automatically Added VBB Framework Code - do not remove
	...
	*/
	...
	/*static final int A = 150;		// Head - Track 0 Calibration zone
	static final int B = 45;		// Head - Track 1
	static final int C = 205;	 	// Head - Track 2
	static final int D = 490; 		// Head - Track 3
	static final int E = 588;		// Head - Track 4
	static final int F = 765;		// Head - Track 5
	static final int G = 1939;		// Head - Track 6
	static final int H = 1645;		// Tail - Track 1
	static final int I = 1805;		// Tail - Track 2
	static final int J = 2090;		// Tail - Track 3
	static final int K = 2188;		// Tail - Track 4
	static final int L = 2365;		// Tail - Track 5
	static final int M = 339; 		// Tail - Track 6 

	static final int A = 0;			// Head - Track 0 Calibration zone
	static final int B = 710;		// Head - Track 1
	static final int C = 800;	 	// Head - Track 2
	static final int D = 890; 		// Head - Track 3
	static final int E = 2310;		// Head - Track 4
	static final int F = 2400;		// Head - Track 5
	static final int G = 2490;		// Head - Track 6
	static final int H = 2490;		// Tail - Track 1
	static final int I = 2400;		// Tail - Track 2
	static final int J = 2310;		// Tail - Track 3
	static final int K = 890;		// Tail - Track 4
	static final int L = 800;		// Tail - Track 5
	static final int M = 710; 		// Tail - Track 6

	int ReferencePosition[] = { A };									// Stepper location of Calibration Track
	int PositionTrackHead[] = { B, C, D, E, F, G };						// Array for Head Tracks
	int PositionTrackTail[] = { H, I, J, K, L, M };						// Array for Tail Tracks

	int PositionTrack[] = { B, C, D, E, F, G, H, I, J, K, L, M };		// Array for all Tracks	  */

	//Define Corresponding Head and Tail
	static final int [] GetTrackTail = { 4, 5, 6, 1, 2, 3 };	// Array for Tail Tracks
	int [] PositionTrack = { 0, 0, 0, 0, 0, 0 };		// Save EEPROM addresses to array - index 0 = calibration position
...
// Programming LCD Keys variables
	...
	// Console Variables
	String printConsole;		// Helper to Print String to Console Window							
	...
	*/
	...
	void setup()
	{
		...
		lcd.createChar (7, degreeMoved);
		/*	
		lcd.createChar (7, reverseY);
		lcd.createChar (8, reverseN);
		*/
	...
	void selectMenu (int selMenu)
	{
		...
		switch (selMenu)
		{
			...
			case 1:
				...
				printToConsole(lcdRowA, lcdRowB);
				delay(500);
				lcdRowA = String("U Head  | Tail D");
				lcdRowB = String("L     Move     R");
				printToLCD(lcdRowA, lcdRowB);
				printToConsole(lcdRowA, lcdRowB);
				/*
				lcd.clear ();
	...
	void selectProgrammedTargetTracks()
	{
		...
		if (key != lastKey)
		{
			switch (key) {
				...
				//	lcdRowB = String("Moving +10 steps");
